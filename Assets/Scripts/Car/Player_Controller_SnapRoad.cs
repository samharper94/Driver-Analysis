...
public class Player_Controller_SnapRoad : MonoBehaviour
{
    ...
    Vector3 CarPos; //shaking the car
    Vector3 standard_camera_orientation;
    Vector3 camera_orientation;
    public InputField BrakePower; //changing the power of the brakes
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void Start()
    {
        ...
        SetSpeedDisplay();
        BrakePower.text = "10"; //Initial brake power of 10
    ...
    void FixedUpdate() //Update calls before rendering frame (FPS dependent), FixedUpdate is dependent on physics calculations
    {
        ...
        brakemultiplier = float.Parse(BrakePower.text); //take brake power from text box
        brake = (Input.GetAxis("Brake")+1f) * rb.mass * brakemultiplier; //calculate braking force
        ...
        velocitydisp = rb.velocity.magnitude * 3.6f; //convert to km/h
        SetSpeedDisplay(); //update speed display
    }
    ...
    void SetSpeedDisplay()
    {
        ...
        steeringinputdisp.value = steeringinput; //show steering input on bar
        float throttleinput = Input.GetAxis("Throttle");
        throttleinputdisp.value = throttleinput; //show throttle input on bar
        float brakeinput = Input.GetAxis("Brake");
        brakeinputdisp.value = brakeinput; //show brake input on bar
    }
    ...
    void Update()
    {
        ...
        float current_time = Time.time; //log current time
        float old_time = 0f;
        do
        {
            ...
            {
                file.WriteLine(lines); //write line to file without overwriting previous lines
            }
